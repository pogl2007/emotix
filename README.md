...
Бэкенд и фронт поднимаются одной командой. Модель в образ не входит —
положи обученный чекпоинт в `backend/models/best_emotix.pt`, он монтируется
томом только на чтение.
...
Порты меняются переменными, если 3000 или 8000 заняты:

```bash
BACKEND_PORT=8001 FRONTEND_PORT=3300 docker compose up --build
```

Что внутри и почему так:

- **backend** — `python:3.12-slim`, torch ставится CPU-сборкой с отдельного
  индекса PyTorch: без этого pip тянет CUDA-колёса на ~2 ГБ. `facenet-pytorch`
  жёстко требует `torch<2.3` и при обычной установке снёс бы CPU-сборку,
  поэтому он ставится с `--no-deps`. Веса MTCNN лежат внутри пакета, в сеть
  при старте не ходит. Запускается от непривилегированного пользователя,
  healthcheck по `/health`.
- **frontend** — трёхстадийная сборка на `node:20-alpine` в режиме
  `output: "standalone"`: в финальный образ попадают `server.js` и только
  нужные модули. Фронт стартует после того, как бэкенд прошёл healthcheck.

Одна тонкость: `NEXT_PUBLIC_API_URL` вшивается в клиентский JS на этапе
**сборки**, а запросы к API идут из браузера пользователя. Поэтому там адрес,
видимый снаружи (`http://localhost:8000`), а не имя сервиса `backend` из
docker-сети — и при смене `BACKEND_PORT` фронт нужно пересобрать
(`--build` это делает).

Без чекпоинта всё тоже поднимется: сайт откроется, в шапке будет
`MODEL OFFLINE`, а анализ вернёт 503 с подсказкой.
